...
namespace MediaDevices
{
    ...
    public sealed class MediaDevice : IDisposable
    {
        ...
        static MediaDevice()
        {
            DeviceManager = PortableDeviceManager.New(); 
            // ReSharper disable once SuspiciousTypeConversion.Global
            ServiceManager = (IPortableDeviceServiceManager)(object)DeviceManager;
        }
        ...
        private MediaDevice(string deviceId)
        {
            ...
            uint count = 256;
            var sb = new char[(int)count];
            try
            {
                count = 256;
                DeviceManager.GetDeviceDescription(deviceId, sb, ref count);
                this.Description = new string(sb, 0, (int)count);
            }
            ...
            try
            {
                ...
                this._friendlyName = new string(sb, 0, (int)count);
            }
            catch (COMException ex)
            {
                Trace.WriteLine(ex.ToString());
            ...
            try
            {
                ...
                this.Manufacturer = new string(sb, 0, (int)count);
            }
            catch (COMException ex)
            {
                Trace.WriteLine(ex.ToString());
            ...
            this.device = PortableDevice.New();
        }
        ...
        public string FriendlyName
        {
            ...
            set
            {
                ...
                IPortableDeviceValues devInValues = PortableDeviceValues.New();
...
#pragma warning restore IDE0059 // Unnecessary assignment of a value
                ...
                try
                {
                    ...
                    uint count = 256;
                    DeviceManager.GetDeviceFriendlyName(this.DeviceId, buffer, ref count);
                    this._friendlyName = new string(buffer, 0, (int)count);
                }
                catch (COMException ex)
                {
                    Trace.WriteLine(ex.ToString());
        ...
        public void Connect(MediaDeviceAccess access = MediaDeviceAccess.Default, MediaDeviceShare share = MediaDeviceShare.Default, bool enableCache = false)
        {
            ...
            var clientInfo = PortableDeviceValues.New();
            ...
            this.device.Capabilities(out this._deviceCapabilities);
            this.device.Content(out this.deviceContent);
            this.deviceContent.Properties(out this.deviceProperties);
            this.deviceProperties.GetValues(Item.RootId, null, out this._deviceValues);

            ComTrace.WriteObject(this._deviceValues);

            // advice event handler
            this._eventCallback = new EventCallback(this);
            this.device.Advise(0, this._eventCallback, null, out this._eventCookie);
        ...
        public void Disconnect()
        {
            ...
            {
                this.device.Unadvise(this._eventCookie);
        ...
        {
            if (!TryGetDirectoryInfo(path, out var directory))
            {
        ...
        /// <exception cref="System.IO.DirectoryNotFoundException">path is invalid.</exception>
        /// <exception cref="MediaDevices.NotConnectedException">device is not connected.</exception>
        public bool TryGetDirectoryInfo(string path, [NotNullWhen(true)] out MediaDirectoryInfo? directory)
        {
            if (path == null)
            {
                throw new ArgumentNullException(nameof(path));
            ...
            directory = new MediaDirectoryInfo(this, item);
            return true;
        }
        ...
        public Stream OpenReadFromPersistentUniqueId(string persistentUniqueId)
        {
            ...
            var item = Item.GetFromPersistentUniqueId(this, persistentUniqueId);
        ...
        //{
        //    var values = PortableDeviceValues.New();
        ...
        public MediaStorageInfo? GetStorageInfo(string storageObjectId)
        {
            ...
            IPortableDeviceKeyCollection keys = PortableDeviceKeyCollection.New();
        ...
        public IEnumerable<MediaDeviceService> GetServices(MediaDeviceServices service)
        {
            ...
            uint num = 0;
            ServiceManager.GetDeviceServices(this.DeviceId, serviceGuid, null, ref num);
            ...
            ServiceManager.GetDeviceServices(this.DeviceId, serviceGuid, services, ref num);
