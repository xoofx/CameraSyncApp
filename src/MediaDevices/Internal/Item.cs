...
namespace MediaDevices.Internal
{
    ...
    internal class Item
    {
        ...
        static Item()
        {
            ...
            keyCollection = PortableDeviceKeyCollection.New();
        ...
        public static Item? GetFromPersistentUniqueId(MediaDevice device, string persistentUniqueId)
        {
            ...
            var collection = PortableDevicePropVariantCollection.New();

            using (var propVariantPUID = PropVariantFacade.StringToPropVariant(persistentUniqueId))
            {
                collection.Add(propVariantPUID.Value);
            }
        ...
        private Item(MediaDevice device, string id, string? path)
        {
            ...
            else
            {
                ...
                {
                    string p = GetPath();
                    this._path = Path.GetDirectoryName(p);
                    this.FullName = p;
                }
            }
        }
        ...
        public void Refresh() 
        {
            if (this.Id != Item.RootId)
            {
                ...
                if (this._path != null) // TODO check if we can remove empty pathes
                {
                    // don't use Path.Combine
                    this.FullName = this._path.TrimEnd(DirectorySeparatorChar) + DirectorySeparatorChar + this.Name;
                }
            }
        }
        ...
        private void GetProperties()
        {
            ...
            try
            {
                ...
                this._device.deviceProperties!.GetValues(this.Id, keyCollection, out values);
            }
            catch (Exception ex)
            {
        ...
        public Item? Parent
        {
            get
            {
                ...
                {
                    this._parent = string.IsNullOrEmpty(this.ParentId) ? null : new Item(this._device, this.ParentId, Path.GetDirectoryName(Path.GetDirectoryName(this.FullName)));
                }
                return this._parent;
            }
        }
        ...
        {
            this._device.deviceContent!.EnumObjects(0, this.Id, null, out IEnumPortableDeviceObjectIDs? enumerator);
            if (enumerator == null)
            {
            ...
            while (fetched > 0)
            {
                for (int index = 0; index < fetched; index++)
                {
                    ...
                    try
                    {
                        item = Item.Create(this._device, objectIds[index], this.FullName);
                    }
        ...
        internal Item? CreateSubdirectory(string path)
        {
            ...
            foreach (var folder in folders)
            {
                ...
                if (child == null)
                {
                    ...
                    IPortableDeviceValues deviceValues = PortableDeviceValues.New();
                    ...
                    try
                    {
                        this._device.deviceContent!.CreateObjectWithPropertiesOnly(deviceValues, ref id);
                    }
                    catch (Exception ex)
                    {
                    ...
                    child = Item.Create(this._device, id!, parent.FullName);
                }
        ...
        {
            var objectIdCollection = PortableDevicePropVariantCollection.New();

            var propVariantValue = PropVariantFacade.StringToPropVariant(this.Id);
            objectIdCollection.Add(propVariantValue.Value);

            IPortableDevicePropVariantCollection results = PortableDevicePropVariantCollection.New();
            // TODO: get the results back and handle failures correctly
            
            this._device.deviceContent!.Delete(recursive ? PORTABLE_DEVICE_DELETE_WITH_RECURSION : PORTABLE_DEVICE_DELETE_NO_RECURSION, objectIdCollection, ref results);
        ...
        public string GetPath()
        {
            ...
            do
            {
                ...
                if (string.IsNullOrWhiteSpace(item.ParentId))
                {
                    ...
                    if (item == null)
                    {
                        throw new Exception($"Problem occurred when trying to get full object path on device {this._device.FriendlyName}.");
                    }
                }
            ...
            } while (!(item = new Item(this._device, item.ParentId!)).IsRoot);
            return sb.ToString();
        }
        ...
        private Item? TryHandleNonHierarchicalStorage()
        {
            ...
            // directory.
            var drives = this._device.GetDrives();
        ...
        {
            this._device.deviceContent!.Transfer(out IPortableDeviceResources resources);

            IStream wpdStream;
            uint optimalTransferSize = 0;

            resources.GetStream(this.Id, WPD.RESOURCE_DEFAULT, 0, ref optimalTransferSize, out wpdStream);
        ...
        internal Stream OpenReadThumbnail()
        {
            ...
            resources.GetStream(this.Id, WPD.RESOURCE_THUMBNAIL, 0, ref optimalTransferSize, out wpdStream);
        ...
        internal Stream OpenReadIcon()
        {
            ...
            resources.GetStream(this.Id, WPD.RESOURCE_ICON, 0, ref optimalTransferSize, out wpdStream);
        ...
        internal void UploadFile(string fileName, Stream stream)
        {
            ...
            IPortableDeviceValues portableDeviceValues = PortableDeviceValues.New();
            ...
            using (PropVariantFacade now = PropVariantFacade.DateTimeToPropVariant(DateTime.Now))
            {
                ...
                uint num = 0u;
                string? text = null;
                this._device.deviceContent!.CreateObjectWithPropertiesAndData(portableDeviceValues, out IStream wpdStream, ref num, ref text);
        ...
        internal bool Rename(string newName)
        {
            ...
            this._device.deviceProperties!.SetValues(this.Id, portableDeviceValues, out result);
            ...
            if (result.TryGetStringValue(WPD.OBJECT_ORIGINAL_FILE_NAME, out string? check))
            {
                if (check == "Error: S_OK")
                {
                    ...
                    var newItem = this._parent!.GetChildren().FirstOrDefault(i => _device.EqualsName(i.Name, newName));
                    this.Id = newItem!.Id;
